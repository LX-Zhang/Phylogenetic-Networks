...
*  over the given nubmer of leaves with the given number of 
*  reticuation nodes.
...
*  outdegree < 1 if u \in B,  <2 if u \in A or a root.  
* (2) v is randomle selected among all the nodes with indegree
*  < 2 if v\in B, <1 if v\in A and <0 if v\in C.
* The program stops untill all the nodes satisfy the degree 
* constraint. 
*/
...
if (argc >=4) { n_l=atoi(argv[1]); n_r=atoi(argv[2]); }
...
srand(time(NULL));
 ...
 for (j=0; j<atoi(argv[3]); j++) {
  ...
  do {
		 success=1;
		 /* printf("do loop\n"); */
        for (i=0; i<n+n_l; i++){ nodes_type[i]=1;}
        /* randomly selected n_r nodes as reticulation nodes */
        for (i=0; i<n_r; i++) {   
		   k=rand()%(n-2); 
		   while (nodes_type[k+2]==-1){k=rand()%(n-2);} 
           nodes_type[2+k]=-1;
        }
        for (i=0; i<n; i++) printf("%d ", nodes_type[i]);
        printf("\n");
       ...
       for (i=1; i<n; i++){
        ...
        for (i=n; i<n+n_l; i++) { in_copy[k_in]=i; k_in +=1; }


        printf("k_out %d  k_in %d\n", k_out, k_in);
          for (i=0; i<k_out; i++){  printf("%d ", out_copy[i]);}
            printf("\n");
          for (i=0; i<k_in; i++){  printf("%d ", in_copy[i]);}
        printf("\n");



        no_edges=0; Edges[0][0]=0; Edges[0][1]=1; no_edges +=1;
      /* printf("Edges\n %d  %d\n", 0, 1); */
        out_copy[0]=-1; in_copy[0]=-1; 
       ...
       for (i=1; i<k_in; i++) {
	      ...
	      for (count=0; count<k_out; count++) {
		    ...
		    if (out_copy[count] !=-1 && out_copy[count]> node2) break;
	      }
	      if (flag == 0) { success=0; break; }
	 /*
	 printf("in inifite look node2=%d, flag=%d\n", node2,  flag);
	...
	*/
	    ...
	    test=0; 
        for ( ; ; ) {
		 test +=1;  if (test==30) { success=0; break;}
	     r=rand()%flag;
		 /*
		  ...
		  */
		   ...
		   /* printf("%d  %d\n", node1, node2); */	
		   Edges[no_edges][0]=node1; Edges[no_edges][1]=node2;
		      no_edges +=1;
		   break;
	     }
        }
	  /* printf("out infinit_loop=\n"); */
    } /* end for loop */
  } while (success==0);
